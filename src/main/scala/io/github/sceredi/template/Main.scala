...
@main def hello(): Unit =
  ...
  val minReading =
    if proximityReadings.nonEmpty then proximityReadings.foldLeft(1.0)((acc, r) => if r < acc then r else acc)
    else 1.0
  println("Minimum distance to obstacle: " + minReading.toString)

  if minReading < 0.3 then println("⚠️  OBSTACLE DETECTED - Robot will turn away from obstacle!")
  else println("✅ No obstacles nearby - Robot will perform random walk")
